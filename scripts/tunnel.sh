...
# Constants — tunnel points to Vite dev server, which proxies /api and /webhook to backend
...
stop_tunnel() {
    ...
    rm -f "$NGROK_LOG_FILE"
    echo ""
    echo "💡 Next: bun run webhook:clear"
}
...
status_tunnel() {
    ...
    if [ -z "$TUNNEL_INFO" ]; then
        ...
        echo ""
        echo "💡 Start: bun run tunnel:start"
        return 1
    fi
...
case "$1" in
    ...
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  start   - Start ngrok tunnel on port 5173 (Vite frontend, proxies API to backend)"
