...
export async function handleWebhook(c: Context) {
  ...
  bot.command("start", async (ctx: GrammyContext) => {
    ...
    if (typeof payload === "string" && payload.startsWith("join_")) {
      ...
      if (alreadyMember) {
        await ctx.reply(
          `You're already in "${group.name}"! Open the app to see your expenses.`,
          {
            reply_markup: {
              inline_keyboard: [
                [
      ...
      await ctx.reply(
        `You've joined "${group.name}"! Open the app to start splitting expenses.`,
        {
          reply_markup: {
            inline_keyboard: [
              [
  ...
  bot.on("message:text", async (ctx: GrammyContext) => {
    if (ctx.message && !ctx.message.text?.startsWith("/")) {
      await ctx.reply(
        "Use the button below to open Splitogram.",
        {
          reply_markup: {
            inline_keyboard: [
              [
