...
- Store wallet address in backend: `PUT /v1/users/me/wallet` with `{ address }` after connection
...
1. **Frontend** calls `GET /v1/settlements/:id/tx` — backend returns transaction parameters
2. **Backend** constructs the Jetton transfer payload:
   - Looks up creditor's wallet address from `users` table
   - USDT Jetton contract address (testnet)
...
3. **Frontend** sends transaction via TON Connect:
   ```typescript
   const tx = {
   ...
   ```
4. **Frontend** receives result (BOC or tx hash), sends to backend: `POST /v1/settlements/:id/verify`
5. **Backend** updates settlement status: `open → payment_pending`
...
- Endpoint: `POST /v1/settlements/:id/verify` with `{ boc }` or `{ tx_hash }`
...
- On failure: rollback `payment_pending → open`, return clear error
- **Timeout:** `AbortSignal.timeout(10000)` on TONAPI calls (playbook: explicit timeout on every I/O)
- **Polling:** If tx not immediately confirmed, return "pending" to frontend. Frontend polls `GET /v1/settlements/:id` every 3s for up to 60s.
...
- Endpoint: `POST /v1/settlements/:id/mark-external`
