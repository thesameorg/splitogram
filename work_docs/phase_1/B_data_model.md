...
**API prefix:** All endpoints under `/v1` (playbook principle)
...
- Endpoint: `POST /v1/auth` → returns session token + user info
...
- Endpoints:
  - `POST /v1/groups` — create group, creator auto-joined with role 'creator'
  - `GET /v1/groups` — list user's groups (with net balance per group)
  - `GET /v1/groups/:id` — group detail with members and balances
  - `POST /v1/groups/:id/join` — join via invite code
  - `GET /v1/groups/join/:invite_code` — resolve invite code (used by deep links)
...
- Endpoints:
  - `POST /v1/groups/:id/expenses` — create expense, body: `{ paid_by, amount, description, participant_ids[] }`
  - `GET /v1/groups/:id/expenses` — list expenses for group (paginated, most recent first)
...
- Endpoints:
  - `GET /v1/groups/:id/balances` — optimized debt graph for the group
  - `GET /v1/groups/:id/balances/me` — what the current user owes / is owed
...
- Endpoint: `POST /v1/expenses/direct` — body: `{ other_user_telegram_id, amount, description }`
