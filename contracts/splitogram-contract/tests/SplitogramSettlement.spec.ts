...
describe('SplitogramSettlement', () => {
  ...
  beforeEach(async () => {
    ...
    contract = blockchain.openContract(await SplitogramSettlement.fromInit(owner.address, 100n));
  ...
  async function sendSettlement(
    ...
    const forwardPayload = beginCell().storeUint(0, 32).storeAddress(recipient).endCell().asSlice();
  ...
  it('should reject settlement if jetton wallet not configured', async () => {
    ...
    const fresh = blockchain.openContract(await SplitogramSettlement.fromInit(owner.address, 100n));
    // Different init params to get a different address
    const fresh2 = blockchain.openContract(await SplitogramSettlement.fromInit(owner.address, 99n));
    await fresh2.send(
      owner.getSender(),
      { value: toNano('0.5') },
      { $$type: 'Deploy', queryId: 0n },
  ...
  it('should reject invalid forward_payload (wrong op)', async () => {
    const badPayload = beginCell().storeUint(1, 32).storeAddress(userB.address).endCell().asSlice();
